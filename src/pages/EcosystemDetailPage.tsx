...
export function EcosystemDetailPage() {
  ...
  useEffect(() => {
    ...
    setEcosystemCssVars(containerRef.current, ecosystem.color || '#91A6B8', isDark);
  ...
  const accentBase = ecosystem.color || 'var(--alpha-info)';
    ...
    <DashboardLayout disableMonochrome>
      ...
      >
        ...
        <Button
          ...
          className={`mb-6 font-mono macos-btn macos-btn--ghost ${isDark ? 'text-[var(--alpha-text-muted)] hover:text-[var(--alpha-text)]' : 'text-[var(--alpha-text-muted)] hover:text-[var(--alpha-text)]'}`}
        >
        ...
        >
          ...
          <div className="relative flex flex-col md:flex-row md:items-center gap-6">
            <div
              ...
              style={{
                background: isDark ? 'var(--mac-backdrop)' : 'var(--alpha-surface-soft)',
            ...
            <div className="flex-1">
              <h1 className={`text-3xl font-bold font-mono mb-2 ${isDark ? 'text-[var(--alpha-text)]' : 'text-[var(--alpha-text)]'}`}>
                {ecosystem.name}
              </h1>
              <a
                ...
                className={`text-sm font-mono flex items-center gap-2 transition-colors ${isDark ? 'text-[var(--alpha-text-muted)] hover:text-[var(--alpha-signal)]' : 'text-[var(--alpha-text-muted)] hover:text-[var(--alpha-signal)]'}`}
              >
          ...
          <div className="mt-6">
            <div className={`h-3 rounded-full overflow-hidden border ${isDark ? 'bg-[var(--alpha-surface-strong)] border-[var(--alpha-border)]' : 'bg-[var(--alpha-surface-soft)] border-[var(--alpha-border)]'}`}>
              <div
        ...
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="relative max-w-md group flex-1">
            ...
            <Input
              ...
              className={`pl-10 font-mono macos-input border-2 transition-all ${isDark ? 'bg-[var(--alpha-surface-strong)] border-[var(--alpha-border)] text-[var(--alpha-text)] focus:border-[var(--alpha-signal)]' : 'bg-[var(--alpha-surface-soft)] border-[var(--alpha-border)] text-[var(--alpha-text)] focus:border-[var(--alpha-signal)]'}`}
            />
          </div>

          <Button
            onClick={() => setIsAddModalOpen(true)}
            className={`font-mono macos-btn macos-btn--primary border-2 transition-all duration-200 ${isDark ? 'bg-[var(--alpha-signal)] text-[var(--alpha-accent-contrast)] border-[var(--alpha-signal)] hover:bg-[var(--alpha-signal-press)]' : 'bg-[var(--alpha-signal)] text-[var(--alpha-accent-contrast)] border-[var(--alpha-signal)] hover:bg-[var(--alpha-signal-press)]'}`}
          >
        ...
        {filteredAssigned.length === 0 ? (
          <div className="py-16 text-center border-2 border-dashed rounded-xl border-[var(--alpha-border)] macos-card macos-panel">
            <LayoutGrid className={`w-16 h-16 mx-auto mb-4 ${isDark ? 'text-[var(--alpha-text)]' : 'text-[var(--alpha-text)]'}`} />
            <h3 className={`font-mono font-bold mb-2 ${isDark ? 'text-[var(--alpha-text)]' : 'text-[var(--alpha-text)]'}`}>No Projects Yet</h3>
            ...
            <Button
              onClick={() => setIsAddModalOpen(true)}
              className={`font-mono macos-btn macos-btn--primary border-2 ${isDark ? 'bg-[var(--alpha-signal)] text-[var(--alpha-accent-contrast)] border-[var(--alpha-signal)]' : 'bg-[var(--alpha-signal)] text-[var(--alpha-accent-contrast)] border-[var(--alpha-signal)]'}`}
            >
              <Plus className="w-4 h-4 mr-2" />
              Add Projects
            </Button>
            ...
            {filteredAssigned.map((project, index) => {
              ...
              return (
                <div
                  key={project.id}
                  className={`
                    relative p-4 rounded-xl border overflow-hidden group macos-card-entry
                  ...
                  `}
                  style={{ borderLeft: `3px solid var(--ecosystem-accent, ${accentBase})`, ['--mac-delay' as any]: `${index * 26}ms` }}
                >
                  <div
                  ...
                  <button
                    ...
                    className={`absolute top-2 right-2 z-20 p-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-200 ${
                      isDark ? 'bg-[var(--alpha-surface-strong)] text-[var(--alpha-text-muted)] hover:text-[var(--alpha-danger)]' : 'bg-[var(--alpha-surface-soft)] text-[var(--alpha-text-muted)] hover:text-[var(--alpha-danger)]'
                    }`}
                  ...
                  <div className="relative">
                    <div className="flex items-start gap-3 mb-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className={`font-mono font-bold text-sm truncate ${isDark ? 'text-[var(--alpha-text)]' : 'text-[var(--alpha-text)]'}`}>
                          {project.projectName}
                        </h4>
                        <span
                    ...
                    <div className="mb-3">
                      <div className={`flex items-center justify-between text-[10px] font-mono mb-1 ${isDark ? 'text-[var(--alpha-text-muted)]' : 'text-[var(--alpha-text-muted)]'}`}>
                        <span>Progress</span>
                        <span style={{ color: 'var(--ecosystem-accent, ' + accentBase + ')' }}>{projectProgress}%</span>
                      </div>
                      <div className={`h-1.5 rounded-full overflow-hidden ${isDark ? 'bg-[var(--alpha-surface-strong)]' : 'bg-[var(--alpha-surface-soft)]'}`}>
                    ...
                    <Button variant="outline" size="sm" asChild className={`w-full font-mono text-[10px] border transition-all ${isDark ? 'border-[var(--alpha-border)] text-[var(--alpha-text-muted)] hover:bg-[var(--alpha-signal-soft)] hover:text-[var(--alpha-signal)]' : 'border-[var(--alpha-border)] text-[var(--alpha-text-muted)] hover:bg-[var(--alpha-signal-soft)] hover:text-[var(--alpha-signal)]'}`}>
          ...
          <div className={`w-full max-w-2xl max-h-[80vh] overflow-hidden rounded-xl border-2 ${isDark ? 'bg-[var(--alpha-surface)] border-[var(--alpha-signal-border)]' : 'bg-[var(--alpha-panel)] border-[var(--alpha-signal-border)]'}`}>
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className={`text-xl font-mono font-bold ${isDark ? 'text-[var(--alpha-text)]' : 'text-[var(--alpha-text)]'}`}>
                    Add Projects to {ecosystem.name}
                  </h2>
                  <p className={`text-sm font-mono mt-1 ${isDark ? 'text-[var(--alpha-text-muted)]' : 'text-[var(--alpha-text-muted)]'}`}>
                    {availableProjects.length} projects available
                  </p>
                </div>
                <button onClick={() => setIsAddModalOpen(false)} className={`p-2 rounded-lg transition-colors ${isDark ? 'hover:bg-[var(--alpha-hover-soft)] text-[var(--alpha-text-muted)]' : 'hover:bg-[var(--alpha-hover-soft)] text-[var(--alpha-text-muted)]'}`} aria-label="Close add projects">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              {availableProjects.length === 0 ? (
                <div className="text-center py-8">
                  <p className={`font-mono ${isDark ? 'text-[var(--alpha-text-muted)]' : 'text-[var(--alpha-text-muted)]'}`}>
                ...
                <div className="space-y-2 max-h-[50vh] overflow-y-auto">
                  {availableProjects.map((project) => (
                    <div
                      key={project.id}
                      className={`flex items-center justify-between p-3 rounded-lg border ${isDark ? 'bg-[var(--alpha-surface-strong)] border-[var(--alpha-border)] hover:border-[var(--alpha-signal-border)]' : 'bg-[var(--alpha-surface-soft)] border-[var(--alpha-border)] hover:border-[var(--alpha-signal-border)]'}`}
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-lg overflow-hidden border ${isDark ? 'bg-[var(--alpha-surface)] border-[var(--alpha-border)]' : 'bg-[var(--alpha-surface)] border-[var(--alpha-border)]'}`}>
                          {project.projectLogo ? (
                            <img loading="lazy" src={project.projectLogo} alt={project.projectName} className="w-full h-full object-cover" />
                          ) : (
                            <div className={`w-full h-full flex items-center justify-center font-bold ${isDark ? 'text-[var(--alpha-signal)]' : 'text-[var(--alpha-signal)]'}`}>
                              {project.projectName[0].toUpperCase()}
                            </div>
                          )}
                        </div>
                      ...
                      <Button
                        ...
                        className={`font-mono text-xs macos-btn macos-btn--primary ${isDark ? 'bg-[var(--alpha-signal)] text-[var(--alpha-accent-contrast)] hover:bg-[var(--alpha-signal-press)]' : 'bg-[var(--alpha-signal)] text-[var(--alpha-accent-contrast)] hover:bg-[var(--alpha-signal-press)]'}`}
                      >
